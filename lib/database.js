...
var setup = function(req, res, callback) {
  ...
  if (typeof dataset !== 'undefined' && dataset !== null) {
    ...
    dataset = auth['bearer'];
    if (typeof dataset !== 'undefined' && dataset !== null) {
      sql = 'SELECT * FROM account WHERE bearer_token = ? OR oauth_proxy = ?';
    }
  }
  ...
  pool.getConnection(function(err, connection) {
    ...
    connection.query(sql, [dataset, dataset], function(err, rows) {
      if (err) {
        connection.release();
        utils.reportServerError(res, err);
        return;
      ...
      // Validate signature if OAuth1.0a
      if (typeof auth.oauth_consumer_key !== 'undefined') {
      ...
      // NEED TO VERIFY EXPIRES FOR BEARER TOKENS
